...
make TEXMF=/usr/share/texmf install-tex 2>&1 | tee ${PKG_LOGDIR}/install-tex.locally.log

make -j1 DESTDIR=${PKG_INSTALL_DIR} install 2>&1 | tee ${PKG_LOGDIR}/install.package.log
make DESTDIR=${PKG_INSTALL_DIR} TEXMF=/usr/share/texmf install-tex 2>&1 | tee ${PKG_LOGDIR}/install-tex.package.log
